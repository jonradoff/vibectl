...
const LOG_TYPE_LABELS: Record<string, { label: string; color: string; icon: string }> = {
  ...
  plan_received: { label: 'Plan Received', color: 'text-cyan-400', icon: 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15a2.25 2.25 0 0 1 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25Z' },
  plan_accepted: { label: 'Plan Accepted', color: 'text-emerald-400', icon: 'M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z' },
  plan_rejected: { label: 'Plan Rejected', color: 'text-orange-400', icon: 'm9.75 9.75 4.5 4.5m0-4.5-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z' },
}
        ...
        <div className="space-y-1">
          {entries.map((entry: ActivityLogEntry) => (
            <ActivityLogRow
              key={entry.id}
              entry={entry}
              project={entry.projectCode ? projectMap.get(entry.projectCode) : undefined}
            />
          ))}
        </div>
      )}
