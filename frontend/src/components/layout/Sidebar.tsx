        ...
        {currentUser ? (
          <div className="flex items-center gap-2.5">
            <NavLink to="/profile" className="flex items-center gap-2.5 flex-1 min-w-0 group">
              <div className="w-7 h-7 rounded-full bg-indigo-600/40 flex items-center justify-center text-indigo-300 font-semibold text-xs shrink-0">
                {currentUser.displayName.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-xs font-medium text-gray-300 group-hover:text-white truncate transition-colors">{currentUser.displayName}</p>
                <p className="text-[10px] text-gray-600 truncate">
                  {currentUser.email || (currentUser.githubUsername ? `@${currentUser.githubUsername}` : currentUser.globalRole)}
                </p>
              </div>
            </NavLink>
            <button
