...
# Cache-Optimizer Trace Recording (research toggle)

When `VIBECTL_RECORD_TRACES=1`, each Claude Code spawn is wrapped with a local recording sidecar that captures the full Anthropic API request/response (system prompt, tools, messages, assistant response, tool calls, token counts including cache read/creation) in cache-optimizer corpus format. Off by default; no behavior change when unset.

## How it works

For every Claude Code subprocess (initial spawn, `/compact`/`/reload` restarts, `/fresh`), vibectl-server:
1. Allocates an ephemeral local port via `net.Listen("tcp", "127.0.0.1:0")`.
2. Execs the proxy command (`VIBECTL_RECORDING_PROXY_CMD`) with `--port <p> --session-id <spawnID> --output-dir <VIBECTL_TRACE_OUTPUT_DIR>`. The proxy's CWD is `VIBECTL_RECORDING_PROXY_DIR` (typically the cache-optimizer repo root so its package is importable).
3. Waits up to 5 s for the proxy to print `ready on :<port>` on stdout.
4. Sets `ANTHROPIC_BASE_URL=http://127.0.0.1:<p>` on the Claude Code subprocess env. All Anthropic API traffic routes through the proxy.
5. On Claude exit (any cause), SIGTERMs the proxy. The proxy flushes the trace JSON to `<output-dir>/<spawnID>.json` and exits.
...
| `VIBECTL_RECORDING_PROXY_CMD` | unset | Required when recording. Command to run the sidecar, e.g. `python3 -m optimizer.recording_proxy`. Split on whitespace; vibectl appends its own flags. |
| `VIBECTL_RECORDING_PROXY_DIR` | server CWD | Working directory for the proxy exec. |
| `VIBECTL_TRACE_OUTPUT_DIR` | `~/.vibectl/traces/` | Where trace files land. |

## Important properties

- **Non-fatal.** If the proxy fails to start (binary missing, port collision, timeout), vibectl logs a warning and spawns Claude Code without `ANTHROPIC_BASE_URL` — the Claude session works normally, just unrecorded.
- **Per-spawn lifecycle.** One proxy = one trace file. `/compact` and `/reload` start a fresh Claude subprocess, which starts a fresh proxy. Trace files are independent.
- **Spawn ID is the trace file name.** It's a UUID generated by vibectl per spawn (`ChatSession.ID`), not Claude's session ID. Different from vibectl's chat session lifecycle (which can span multiple Claude spawns).
- **Auth is untouched.** The proxy forwards `authorization`, `anthropic-version`, `anthropic-beta`, and all `x-stainless-*` headers byte-for-byte. Cache behavior is unaffected, which is the point.
- **Trace consumer.** Files in `~/.vibectl/traces/` are ingested by cache-optimizer via `python -m optimizer.convert_traces ~/.vibectl/traces/ --output STATE/corpus/real/`. Convert handles the move from recording dir to corpus dir.
